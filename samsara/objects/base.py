...
from samsara.objects import fields as obj_fields
...
class SamsaraObjectRegistry(ovoo_base.VersionedObjectRegistry):
    def registration_hook(self, cls, index):
        # NOTE(danms): This is called when an object is registered,
        # and is responsible for maintaining samsara.objects.$OBJECT
...
class SamsaraObject(ovoo_base.VersionedObject):
    ...
    # NOTE(danms): This has some minor change between the samsara and o.vo
    ...
    # NOTE(danms): This is samsara-specific
...
class SamsaraObjectDictCompat(ovoo_base.VersionedObjectDictCompat):
    def __iter__(self):
...
class SamsaraObjectSerializer(messaging.NoOpSerializer):
    """A SamsaraObject-aware Serializer.

    This implements the Oslo Serializer interface and provides the
    ability to serialize and deserialize SamsaraObject entities. Any service
    that needs to accept or return SamsaraObjects as arguments or result values
    ...
    def deserialize_entity(self, context, entity):
        if isinstance(entity, dict) and 'samsara_object.name' in entity:
...
def obj_to_primitive(obj):
    ...
    A SamsaraObject becomes a dict, and anything that implements ObjectListBase
    becomes a list.
    """
    if isinstance(obj, ObjectListBase):
        return [obj_to_primitive(x) for x in obj]
    elif isinstance(obj, SamsaraObject):
        result = {}
...
def obj_make_list(context, list_obj, item_cls, db_list, **extra_args):
    ...
    :param:item_cls: The SamsaraObject class of the objects within the list
...
def obj_equal_prims(obj_1, obj_2, ignore=None):
    ...
    if ignore is not None:
        keys = ['samsara_object.changes'] + ignore
    else:
        keys = ['samsara_object.changes']
