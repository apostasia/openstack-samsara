...
samples_dir=$experiment_session_dir"/baseline"
...
# Inicializa Sincroniza os Nodos Físicos
#
echo "+--------------------------------------------------------------+"
...
do
    ...
    #
    echo "+--------------------------------------------------------------+"
    echo "|        Nova: Criando as Máquinas Virtuais com Cirros         |"
    echo "+--------------------------------------------------------------+"
    ...
    do
        nova boot --flavor m2.tiny \
            ...
            --nic net-id=73a01dd8-6cfc-4701-8591-2d986380ebeb \
    ...
    echo "+--------------------------------------------------------------+"
    echo "|           Samsara: Iniciando Karma Workload                  |"
    echo "+--------------------------------------------------------------+"

    for node in ${nodes[@]}
    do
    	echo "[+] Samsara compute node ($node): Iniciando Karma Workload"
    	sshpass -p lups999 ssh lups@$node 'sudo start samsara-local-karma'
    done
    ...
    # Start Experiment Register
    #printf '%s\n' round start end | paste -sd ',' >> $samples_dir/experiment-start_end.csv
    sudo su -c "printf '%s\n' round start end | paste -sd ',' >> $samples_dir/experiment-start_end.csv"
    ...
    for node in ${nodes[@]}
    do
        echo "[-] Samsara compute node ($node): Parando Karma Workload"
    	sshpass -p lups999 ssh lups@$node 'sudo stop samsara-local-karma'
    ...
    round_samples_dir=$samples_dir"/$(date -u +%Y-%m-%dT%H:%M:%S)"
